...
export const DraftController = {
    ...
    openPublishModal(eventId, lat, lng, mapManager, nostrService, journalManager) {
        ...
        if (draftMarker) {
            ...
            if (titleInput) titleInput.value = draftMarker.title || "";
            if (catSelect) catSelect.value = draftMarker.category || "warehouse";
        }
    ...
    saveLocalDraft(title, desc, cat, lat, lng, imageUrls) {
        try {
            ...
            localStorage.setItem('local_draft_storage', JSON.stringify(drafts));
            return true;
        } catch (err) {
    ...
    initPublishLogic(eventId, lat, lng, nostrService, journalManager) {
        ...
        btn.onclick = async () => {
            ...
            btn.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> ${isReadOnly ? 'SAVING LOCALLY...' : 'PUBLISHING...'}`;
            btn.disabled = true;
            ...
            try {
                ...
                if (isReadOnly) {
                    ...
                    if (saved) {
                        showToast("Local draft saved!", "success");
                        DraftController.selectedFiles = [];
                        closeModal();
                ...
                btn.disabled = false;
                btn.innerHTML = isReadOnly ? 'SAVE AS LOCAL' : 'PUBLISH TO NOSTR';
    ...
    initDraftLogic(lat, lng, nostrService, journalManager) {
        ...
        btnSave.onclick = async () => {
            ...
            if (!title) return showToast("⚠️ Title is required", "error");

            const isReadOnly = !AuthManager.canSign();
            btnSave.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> ${isReadOnly ? 'SAVING LOCAL...' : 'SAVING...'}`;
